...
def eval_predictions(data, features, target, train_model = 1,percent = 0.75):
    ...
    clf = RandomForestClassifier(n_jobs=2, random_state=False, warm_start=False)
    if train_model == 1:
...
def analyze(df,features = ['index', 'length'] ,method = 'pam',clusterRange = [2,26],percent = 0.75, iterations=1,train_model=1):
    CM = {}
    actualVals, pred = {}, {}
...
def predict2(df,method,features,clusterRange = [2,26]):
    predictions = {}
    vals = {}
    CM = {}
...
def plotit(vals, preds,title = 'predicion score',score = 'precision',avg='micro',multiplot=False,colour=None):
    if not multiplot:
